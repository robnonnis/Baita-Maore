...
const s = {
  ...
  content: { padding: "24px 20px 60px", animation: "fadeIn 0.3s ease" },
...
const TR = {
  it: {
    ...
    spaBookTitle: "Prenota trattamenti & SPA", spaBookSubtext: "Contattaci per prenotare un trattamento o accedere alla SPA. Gli orari indicati si riferiscono alla disponibilità di accesso alla SPA (sessione da 1 ora):",
  ...
  en: {
    ...
    spaBookTitle: "Book treatments & SPA", spaBookSubtext: "Contact us to book a treatment or access the SPA. The times shown refer to SPA access availability (1 hour session):",
...
function Splash({onDone}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"linear-gradient(160deg,#3d2008,#6b3a1f,#4a2510)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",zIndex:9999,transition:"opacity 0.6s",opacity:fade?0:1,pointerEvents:fade?"none":"all"}}>
      <img src="https://res.cloudinary.com/dovpg47yh/image/upload/v1774558967/IMG_8927_t7ppwk.png" alt="Baita Maore" style={{width:200,filter:"brightness(0) invert(1)",opacity:0.95,marginBottom:24}}/>
      <div style={{fontSize:9,letterSpacing:"5px",textTransform:"uppercase",color:"rgba(244,237,224,0.5)",fontFamily:"'Raleway',sans-serif"}}>Laconi · Sardegna</div>
      <div style={{marginTop:32,width:36,height:2,background:"rgba(244,237,224,0.2)",borderRadius:2,overflow:"hidden"}}>
        <div style={{height:"100%",background:"rgba(244,237,224,0.6)",borderRadius:2,animation:"loadbar 1.8s ease forwards"}}/>
      </div>
...
function ScrollTop() {
  ...
  if(!show) return null;
  return (
    <button onClick={()=>document.getElementById("app-root")?.scrollTo({top:0,behavior:"smooth"})}
      style={{position:"fixed",bottom:24,right:20,width:42,height:42,borderRadius:"50%",background:c.brown,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(107,66,38,0.5)",zIndex:998}}>
      <svg viewBox="0 0 24 24" style={{width:18,height:18,stroke:"white",fill:"none",strokeWidth:2.5}}><polyline points="18 15 12 9 6 15"/></svg>
    </button>
  );
}
  ...
  return <div style={s.app}><FontLink/><LangToggle lang={lang} setLang={setLang}/>
    ...
    <div style={s.content}>
      ...
      <Card><CT text={t.spaBookLabel}/>
        ...
        <a href="https://wa.me/393474029363" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:8,background:"#25D366",color:"white",borderRadius:14,padding:"14px 16px",textDecoration:"none",fontSize:13,marginTop:8}}>
...
function Spa({go,lang,setLang}) {
  ...
  return <div style={s.app}><FontLink/><LangToggle lang={lang} setLang={setLang}/>
    ...
    <div style={s.content}>
      ...
      <Card>
        <CT text={t.spaBookTitle}/>
        <p style={{fontSize:13.5,color:c.muted,marginBottom:12,lineHeight:1.65}}>{t.spaBookSubtext}</p>
        <div style={{marginBottom:8}}>
          <div style={{fontSize:11,letterSpacing:"2px",textTransform:"uppercase",color:c.muted,marginBottom:8}}>{t.morning}</div>
          {["9:30","11:00"].map(x=><span key={x} style={s.spaSlot}>{x}</span>)}
        </div>
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,letterSpacing:"2px",textTransform:"uppercase",color:c.muted,marginBottom:8}}>{t.afternoon}</div>
          {["15:00","16:30","18:00","19:30"].map(x=><span key={x} style={s.spaSlot}>{x}</span>)}
          <span style={s.spaSlot}>{lang==="it"?"21:30 (dopo cena)":"21:30 (after dinner)"}</span>
        </div>
        <a href="mailto:info@baitamaore.it?subject=SPA+Booking" style={{display:"flex",alignItems:"center",gap:8,background:c.brown,color:"white",borderRadius:12,padding:"12px 16px",textDecoration:"none",fontSize:13,marginBottom:8,justifyContent:"center"}}><Ic.mailW/> {t.spaEmailBook}</a>
        <a href="https://wa.me/393474029363" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:8,background:"#25D366",color:"white",borderRadius:12,padding:"12px 16px",textDecoration:"none",fontSize:13,justifyContent:"center"}}>
          <svg viewBox="0 0 24 24" style={{width:16,height:16,fill:"white"}}><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
          {lang==="it"?"Prenota su WhatsApp":"Book on WhatsApp"}
        </a>
      </Card>
...
export default function BaitaMaore() {
  ...
  return (
    <>
      ...
      <div id="app-root" style={{height:"100vh",overflowY:"auto",overflowX:"hidden"}}>
        {screens[screen] || <PH {...props}/>}
      </div>
      <ScrollTop/>
    </>
  );
